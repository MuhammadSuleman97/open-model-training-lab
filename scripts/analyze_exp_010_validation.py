#!/usr/bin/env python3
"""Compare Experiment 010 and its parent on validation data only."""
...
CHILD_RESULT_PATH = PROJECT_ROOT / "artifacts" / "encoder" / "exp-010" / "result.json"
PARENT_MODEL_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-009d" / "selected-model"
CHILD_MODEL_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-010" / "selected-model"
ANALYSIS_PATH = PROJECT_ROOT / "artifacts" / "encoder" / "exp-010" / "validation_transition_analysis.json"
...
if not torch.backends.mps.is_available():
    raise SystemExit("Experiment 010 validation analysis stopped: MPS unavailable.")
if not PARENT_RESULT_PATH.is_file() or not CHILD_RESULT_PATH.is_file():
    raise SystemExit("Experiment 010 validation analysis stopped: finalized models are missing.")
if ANALYSIS_PATH.exists():
    raise SystemExit("Experiment 010 validation analysis stopped: analysis already exists.")
...
if sha256(valid_path) != valid_file["sha256"]:
    raise SystemExit("Experiment 010 validation analysis stopped: validation checksum mismatch.")
...
print("Open Model Training Lab — Experiment 010 validation transition analysis")
