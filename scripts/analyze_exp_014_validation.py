#!/usr/bin/env python3
"""Compare Exp014 with the Exp011 champion on validation only."""
...
CHILD_RESULT_PATH = PROJECT_ROOT / "artifacts" / "encoder" / "exp-014" / "result.json"
PARENT_MODEL_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-011" / "selected-model"
CHILD_MODEL_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-014" / "selected-model"
ANALYSIS_PATH = PROJECT_ROOT / "artifacts" / "encoder" / "exp-014" / "validation_transition_analysis.json"
...
if not torch.backends.mps.is_available():
    raise SystemExit("Experiment 014 validation analysis stopped: MPS unavailable.")
if not PARENT_RESULT_PATH.is_file() or not CHILD_RESULT_PATH.is_file():
    raise SystemExit("Experiment 014 validation analysis stopped: finalized models are missing.")
if ANALYSIS_PATH.exists():
    raise SystemExit("Experiment 014 validation analysis stopped: analysis already exists.")
...
if child_result.get("test_rows_loaded") != 0 or child_result.get("test_evaluated") is not False:
    raise SystemExit("Experiment 014 validation analysis stopped: child reports test access.")
valid_file = data_manifest["files"]["valid"]
valid_path = Path(valid_file["path"])
if sha256(valid_path) != valid_file["sha256"]:
    raise SystemExit("Experiment 014 validation analysis stopped: validation checksum mismatch.")
...
print("Open Model Training Lab — Experiment 014 validation transition analysis")
...
analysis = {
    ...
    "hard_negative_probability_threshold": child_result["hard_negative_probability_threshold"],
