#!/usr/bin/env python3
"""Evaluate Experiment 002 on the fixed balanced BANKING77 test pilot."""
...
EXPERIMENT_RESULT_PATH = (
    PROJECT_ROOT / "experiments" / "exp-002-balanced-539" / "result.json"
)
BASELINE_SUMMARY_PATH = (
    PROJECT_ROOT / "evaluation" / "results" / "pilot_summary.json"
)
ADAPTER_PATH = PROJECT_ROOT / "adapters" / "exp-002-balanced-539"
ADAPTER_FILE = ADAPTER_PATH / "adapters.safetensors"
PREDICTIONS_PATH = (
    PROJECT_ROOT
    / "evaluation"
    / "results"
    / "exp_002_pilot_predictions.jsonl"
)
SUMMARY_PATH = (
...
if pilot_sha256 != pilot_manifest["pilot_sha256"]:
    raise SystemExit("Experiment 002 pilot failed: pilot checksum mismatch.")
if len(pilot) != pilot_manifest["pilot_rows"]:
    raise SystemExit("Experiment 002 pilot failed: pilot row count mismatch.")
if experiment_result["status"] != "complete":
    raise SystemExit("Experiment 002 pilot failed: training is incomplete.")
if sha256(ADAPTER_FILE) != experiment_result["adapter_sha256"]:
    raise SystemExit("Experiment 002 pilot failed: adapter checksum mismatch.")
if baseline_summary["pilot_sha256"] != pilot_sha256:
    raise SystemExit("Experiment 002 pilot failed: stale baseline summary.")
...
if len(completed_positions) != len(predictions):
    raise SystemExit(
        "Experiment 002 pilot failed: duplicate checkpoint rows detected."
    )

for record in predictions:
    if record["adapter_sha256"] != experiment_result["adapter_sha256"]:
        raise SystemExit(
            "Experiment 002 pilot failed: stale adapter results detected."
        )
    if record["prompt_version"] != prompt_config["prompt_version"]:
        raise SystemExit(
            "Experiment 002 pilot failed: stale prompt results detected."
        )
    if record["pilot_sha256"] != pilot_sha256:
        raise SystemExit(
            "Experiment 002 pilot failed: stale pilot results detected."
        )
...
print("Open Model Training Lab — Experiment 002 balanced pilot")
...
if len(predictions) != len(pilot):
    raise SystemExit(
        "Experiment 002 pilot paused with partial results. Rerun the same "
        "command to resume."
    )
...
summary = {
    "name": "exp-002-balanced-539-banking77-pilot-v1",
    "model_id": model_manifest["model_id"],
    "model_revision": model_manifest["resolved_revision"],
    "adapter_sha256": experiment_result["adapter_sha256"],
    "training_rows": experiment_result["training_rows"],
    ...
    "comparison_to_untouched_pilot": comparison,
    **metrics,
}
...
print("exp_002_pilot_evaluation_ok: True")
