#!/usr/bin/env python3
"""Evaluate stable Experiment 003b on the fixed BANKING77 test pilot."""
...
PREDICTIONS_PATH = (
    ...
    / "evaluation"
    / "results"
    / "exp_003b_pilot_predictions.jsonl"
)
SUMMARY_PATH = (
...
if pilot_sha256 != pilot_manifest["pilot_sha256"]:
    raise SystemExit("Experiment 003b pilot failed: pilot checksum mismatch.")
if len(pilot) != pilot_manifest["pilot_rows"]:
    raise SystemExit("Experiment 003b pilot failed: pilot row count mismatch.")
if experiment_result["status"] != "complete":
    raise SystemExit("Experiment 003b pilot failed: training is incomplete.")
if sha256(ADAPTER_FILE) != experiment_result["adapter_sha256"]:
    raise SystemExit("Experiment 003b pilot failed: adapter checksum mismatch.")
if baseline_summary["pilot_sha256"] != pilot_sha256:
    raise SystemExit("Experiment 003b pilot failed: stale baseline summary.")
if exp_002_summary["pilot_sha256"] != pilot_sha256:
    raise SystemExit("Experiment 003b pilot failed: stale Experiment 002.")
...
if len(completed_positions) != len(predictions):
    raise SystemExit(
        "Experiment 003b pilot failed: duplicate checkpoint rows detected."
    )

for record in predictions:
    if record["adapter_sha256"] != experiment_result["adapter_sha256"]:
        raise SystemExit(
            "Experiment 003b pilot failed: stale adapter results detected."
        )
    if record["prompt_version"] != prompt_config["prompt_version"]:
        raise SystemExit(
            "Experiment 003b pilot failed: stale prompt results detected."
        )
    if record["pilot_sha256"] != pilot_sha256:
        raise SystemExit(
            "Experiment 003b pilot failed: stale pilot results detected."
        )
...
print("Open Model Training Lab — Experiment 003b balanced pilot")
...
if len(predictions) != len(pilot):
    raise SystemExit(
        "Experiment 003b pilot paused with partial results. Rerun the same "
        "command to resume."
    )
...
comparison = {
    "untouched": {
        ...
        "invalid_label_rate": baseline_summary["invalid_label_rate"],
        "invalid_label_rate_delta": (
            metrics["invalid_label_rate"]
            - baseline_summary["invalid_label_rate"]
        ),
    },
    "exp_002_balanced_539": {
        "accuracy": exp_002_summary["accuracy"],
        "accuracy_delta": metrics["accuracy"] - exp_002_summary["accuracy"],
        "macro_f1": exp_002_summary["macro_f1"],
        "macro_f1_delta": metrics["macro_f1"] - exp_002_summary["macro_f1"],
        "invalid_label_rate": exp_002_summary["invalid_label_rate"],
        "invalid_label_rate_delta": (
            metrics["invalid_label_rate"]
            - exp_002_summary["invalid_label_rate"]
        ),
    },
}
summary = {
    "name": "exp-003b-balanced-1925-banking77-pilot-v1",
    "model_id": model_manifest["model_id"],
    "model_revision": model_manifest["resolved_revision"],
    "adapter_sha256": experiment_result["adapter_sha256"],
    "training_rows": experiment_result["training_rows"],
...
print(f"untouched_accuracy: {comparison['untouched']['accuracy']:.6f}")
print(
    "accuracy_delta_vs_untouched: "
    f"{comparison['untouched']['accuracy_delta']:+.6f}"
)
print(
    "exp_002_accuracy: "
    f"{comparison['exp_002_balanced_539']['accuracy']:.6f}"
)
print(
    "accuracy_delta_vs_exp_002: "
    f"{comparison['exp_002_balanced_539']['accuracy_delta']:+.6f}"
)
print(f"macro_f1: {summary['macro_f1']:.6f}")
print(
    "macro_f1_delta_vs_untouched: "
    f"{comparison['untouched']['macro_f1_delta']:+.6f}"
)
print(
    "macro_f1_delta_vs_exp_002: "
    f"{comparison['exp_002_balanced_539']['macro_f1_delta']:+.6f}"
)
print(f"invalid_label_rate: {summary['invalid_label_rate']:.6f}")
print(f"peak_memory_gb: {summary['peak_memory_gb']:.3f}")
print(f"predictions: {PREDICTIONS_PATH}")
print(f"summary: {SUMMARY_PATH}")
print("exp_003b_pilot_evaluation_ok: True")
