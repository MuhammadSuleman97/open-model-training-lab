#!/usr/bin/env python3
"""Evaluate the untouched model on the fixed BANKING77 pilot set."""
...
PREDICTIONS_PATH = (
    PROJECT_ROOT / "evaluation" / "results" / "pilot_predictions.jsonl"
)
SUMMARY_PATH = PROJECT_ROOT / "evaluation" / "results" / "pilot_summary.json"
...
def write_predictions(predictions: list[dict[str, object]]) -> str:
    content = "".join(
        ...
        for record in sorted(predictions, key=lambda item: int(item["pilot_position"]))
    )
...
pilot_content = PILOT_PATH.read_bytes()
pilot_sha256 = hashlib.sha256(pilot_content).hexdigest()
if pilot_sha256 != pilot_manifest["pilot_sha256"]:
    raise SystemExit("Pilot evaluation failed: pilot checksum mismatch.")
if len(pilot) != pilot_manifest["pilot_rows"]:
    raise SystemExit("Pilot evaluation failed: pilot row count mismatch.")
...
if len(completed_positions) != len(predictions):
    raise SystemExit("Pilot evaluation failed: duplicate checkpoint rows detected.")

for record in predictions:
    if record["model_revision"] != model_manifest["resolved_revision"]:
        raise SystemExit("Pilot evaluation failed: stale model results detected.")
    if record["prompt_version"] != prompt_config["prompt_version"]:
        raise SystemExit("Pilot evaluation failed: stale prompt results detected.")
    if record["pilot_sha256"] != pilot_sha256:
        raise SystemExit("Pilot evaluation failed: stale pilot results detected.")
...
print("Open Model Training Lab — untouched-model pilot evaluation")
...
if len(predictions) != len(pilot):
    raise SystemExit(
        "Pilot evaluation paused with partial results. Rerun the same command "
        "to resume."
    )

metrics = calculate_metrics(predictions, labels)
summary = {
    "name": "untouched-qwen3-1.7b-banking77-balanced-pilot-v1",
    "model_id": model_manifest["model_id"],
    "model_revision": model_manifest["resolved_revision"],
    "dataset_revision": dataset_manifest["source_revision"],
    "pilot_sha256": pilot_sha256,
...
print("pilot_evaluation_ok: True")
