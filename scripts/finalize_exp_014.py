#!/usr/bin/env python3
"""Select and round-trip Exp014 using validation data only."""
...
TRAINING_RESULT_PATH = PROJECT_ROOT / "artifacts" / "encoder" / "exp-014" / "training_result.json"
RESULT_PATH = PROJECT_ROOT / "artifacts" / "encoder" / "exp-014" / "result.json"
SELECTED_MODEL_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-014" / "selected-model"
...
def stop(message: str) -> None:
    raise SystemExit(f"Experiment 014 finalization stopped: {message}")
...
if data_manifest.get("validation_rows_loaded") != 0 or data_manifest.get("test_rows_loaded") != 0:
    stop("data manifest reports held-out split access.")
if data_manifest.get("labels_rewritten") is not False:
    stop("data manifest does not preserve original labels.")
...
if len(source_records) != int(config["data"]["validation_rows"]):
...
for record in source_records:
    encoded = tokenizer(str(record["text"]), max_length=int(config["tokenization"]["max_length"]), truncation=True)
...
print("Open Model Training Lab — Experiment 014 validation-only finalization")
...
print(f"hard_negative_rows: {training_result['hard_negative_rows']}")
...
if not math.isclose(selected_metrics["accuracy"], float(training_result["best_child_validation_accuracy"]), rel_tol=0.0, abs_tol=1e-12):
    stop("checkpoint validation accuracy does not match training result.")
...
result = {
    ...
    "train_rows": data_manifest["train_rows"],
    "hard_negative_rows": data_manifest["hard_negative_rows"],
    "hard_negative_probability_threshold": data_manifest["hard_negative_probability_threshold"],
    "hard_negative_margin": training_result["hard_negative_margin"],
    "hard_negative_weight": training_result["hard_negative_weight"],
...
print("exp_014_finalization_ok: True")
