#!/usr/bin/env python3
"""Prepare a train-only targeted oversampling manifest for Experiment 010."""
...
OUTPUT_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-010" / "data"
TRAIN_OUTPUT_PATH = OUTPUT_DIR / "train.jsonl"
MANIFEST_PATH = OUTPUT_DIR / "manifest.json"

SEED = 3415
WORST_LABEL_LIMIT = 8
CONFUSION_PAIR_LIMIT = 7
TARGET_MULTIPLIER = 2
...
if TRAIN_OUTPUT_PATH.exists() or MANIFEST_PATH.exists():
    raise SystemExit("Experiment 010 data preparation stopped: output already exists.")
...
if validation_analysis.get("test_rows_loaded") != 0:
    raise SystemExit("Experiment 010 data preparation stopped: validation analysis loaded test data.")

train_file = data_manifest["files"]["train"]
train_path = Path(train_file["path"])
if sha256(train_path) != train_file["sha256"]:
    raise SystemExit("Experiment 010 data preparation stopped: source train checksum mismatch.")
...
if len(source_rows) != data_manifest["rows"]["train"]:
    raise SystemExit("Experiment 010 data preparation stopped: source train row count mismatch.")

target_labels = {
    entry["label"] for entry in validation_analysis["worst_labels"][:WORST_LABEL_LIMIT]
}
for pair in validation_analysis["top_confusions"][:CONFUSION_PAIR_LIMIT]:
    target_labels.add(pair["expected_label"])
    target_labels.add(pair["predicted_label"])

target_rows = [row for row in source_rows if row["label_name"] in target_labels]
if not target_rows:
    raise SystemExit("Experiment 010 data preparation stopped: no target rows found.")
...
manifest = {
    "experiment": "exp-010-deberta-targeted-oversampling",
    "parent_experiment": "exp-009d-deberta-v3-large-float32",
    "parent_validation_accuracy": validation_analysis["finalized_validation_accuracy"],
...
print("Open Model Training Lab — Experiment 010 targeted data preparation")
print(f"seed: {SEED}")
...
print(f"target_rows_added: {len(target_rows) * (TARGET_MULTIPLIER - 1)}")
print(f"train_sha256: {manifest['train_sha256']}")
print(f"manifest: {MANIFEST_PATH}")
print("test_rows_loaded: 0")
print("exp_010_data_preparation_ok: True")
