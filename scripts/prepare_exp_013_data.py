#!/usr/bin/env python3
"""Prepare train-only hard-negative records for Experiment 013."""
...
OUTPUT_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-013" / "data"
...
def stop(message: str) -> None:
    raise SystemExit(f"Experiment 013 data preparation stopped: {message}")
...
if audit_manifest.get("validation_rows_loaded") != 0:
    stop("audit manifest reports validation access.")
if audit_manifest.get("test_rows_loaded") != 0:
    stop("audit manifest reports test access.")
...
if len(train_records) != expected_rows or len(audit_records) != expected_rows:
    stop(
        f"row mismatch: train={len(train_records)} audit={len(audit_records)} "
...
for index, (train_record, audit_record) in enumerate(zip(train_records, audit_records)):
    ...
    model_disagrees = bool(audit_record.get("model_disagrees"))
    active = model_disagrees and predicted_probability >= threshold
    if active:
        active_rows += 1
    prepared.append(
        {
            ...
            "hard_negative_label": (
                audit_record.get("predicted_label") if active else None
            ),
...
if active_rows == 0:
    stop("no hard-negative rows met the configured threshold.")
...
print("Open Model Training Lab — Experiment 013 train-only data preparation")
...
print("exp_013_data_preparation_ok: True")
