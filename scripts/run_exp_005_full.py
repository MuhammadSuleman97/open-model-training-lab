#!/usr/bin/env python3
"""Train one full q/k/v/o LoRA epoch for Experiment 005."""
...
CONFIG_PATH = (
    PROJECT_ROOT / "configs" / "exp-005-attention-qkvo-full.yaml"
)
SFT_MANIFEST_PATH = (
...
for key, value in expected.items():
    if config.get(key) != value:
        raise SystemExit(
            f"Experiment 005 full setup failed: {key} must be {value!r}."
        )
...
if config["lora_parameters"] != {
    ...
    raise SystemExit(
        "Experiment 005 full setup failed: LoRA parameters are unexpected."
    )
if probe_result.get("status") != "complete":
    raise SystemExit(
        "Experiment 005 full setup failed: stability probe is incomplete."
    )
if probe_result.get("nonfinite_adapter_values") != 0:
    raise SystemExit(
        "Experiment 005 full setup failed: probe adapter is non-finite."
    )
if probe_result.get("adapter_value_count") != EXPECTED_ADAPTER_VALUES:
    raise SystemExit(
        "Experiment 005 full setup failed: probe capacity is unexpected."
    )
if probe_result["changed_factor"]["probe"] != targets:
    raise SystemExit(
        "Experiment 005 full setup failed: probe targets do not match."
    )
if probe_result["batch_size"] != config["batch_size"]:
    raise SystemExit(
        "Experiment 005 full setup failed: probe batch size does not match."
    )
if probe_result["learning_rate"] != config["learning_rate"]:
    raise SystemExit(
        "Experiment 005 full setup failed: probe learning rate does not match."
    )
...
if config["iters"] * config["batch_size"] != train_rows:
    raise SystemExit(
        "Experiment 005 full setup failed: configuration is not one epoch."
    )
if config["val_batches"] * config["batch_size"] != valid_rows:
    raise SystemExit(
        "Experiment 005 full setup failed: validation is not complete."
    )
if token_report.get("recommended_max_seq_length") != config["max_seq_length"]:
    raise SystemExit(
        "Experiment 005 full setup failed: sequence length mismatch."
    )
if token_report.get("mask_prompt_safe") is not True:
    raise SystemExit(
        "Experiment 005 full setup failed: prompt masking is not verified."
    )
for name in ("train.jsonl", "valid.jsonl"):
    path = PROJECT_ROOT / config["data"] / name
    if sha256(path) != sft_manifest["files"][name]["sha256"]:
        raise SystemExit(
            f"Experiment 005 full setup failed: {name} checksum mismatch."
        )

model_path = PROJECT_ROOT / config["model"]
if not model_path.is_dir() or not any(model_path.glob("*.safetensors")):
    raise SystemExit(
        "Experiment 005 full setup failed: pinned model is missing."
    )
adapter_path = PROJECT_ROOT / config["adapter_path"]
adapter_file = adapter_path / "adapters.safetensors"
if adapter_path.exists() and any(adapter_path.iterdir()):
    raise SystemExit(
        "Experiment 005 full stopped: adapter directory is not empty."
    )
if RESULT_PATH.exists() or LOG_PATH.exists():
    raise SystemExit(
        "Experiment 005 full stopped: experiment artifact already exists."
    )

if "--check" in sys.argv[1:]:
    if sys.argv[1:] != ["--check"]:
        raise SystemExit("Usage: run_exp_005_full.py [--check]")
    print("Open Model Training Lab — Experiment 005 full preflight")
    print(f"training_rows: {train_rows}")
    print(f"validation_rows: {valid_rows}")
    print("changed_factor: LoRA q/v targets to q/k/v/o targets")
    ...
    print("exp_005_full_preflight_ok: True")
    raise SystemExit(0)
if sys.argv[1:]:
    raise SystemExit("Usage: run_exp_005_full.py [--check]")
...
print("Open Model Training Lab — Experiment 005 q/k/v/o full training")
print("purpose: isolate the effect of expanded LoRA attention targets")
print(f"training_rows: {train_rows}")
print(f"validation_rows: {valid_rows}")
print("batch_size: 1")
...
if interrupted:
    ...
    interrupted_result = {
        "name": "exp-005-attention-qkvo-full",
    ...
    if archived_experiment_dir.exists() or archived_adapter_path.exists():
        raise SystemExit(
            "Experiment 005 full interrupted, but archive target exists. "
    ...
    raise SystemExit(
        "Experiment 005 full interrupted. The process was stopped and partial "
...
if abort_reason is not None:
    ...
    raise SystemExit(
        f"Experiment 005 full safety stop: {abort_reason}. "
        "Do not rerun automatically."
    )
if return_code != 0:
    raise SystemExit(
        f"Experiment 005 full failed with exit code {return_code}. "
        f"Output: {LOG_PATH}"
    )
if not adapter_file.is_file():
    raise SystemExit(
        "Experiment 005 full failed: final adapter is missing."
    )
...
if [report["iteration"] for report in validation_losses] != [1, 4617, 9233]:
    raise SystemExit(
        "Experiment 005 full result capture failed: validations are missing."
    )
if not all(
    ...
    raise SystemExit(
        "Experiment 005 full result capture failed: a loss is non-finite."
    )
if not training_reports or training_reports[-1]["iteration"] != config["iters"]:
    raise SystemExit(
        "Experiment 005 full result capture failed: final report is missing."
    )
...
if adapter_value_count != EXPECTED_ADAPTER_VALUES:
    raise SystemExit(
        "Experiment 005 full result capture failed: capacity is unexpected."
    )
if nonfinite_adapter_values:
    raise SystemExit(
        "Experiment 005 full result capture failed: adapter is non-finite."
    )

result = {
    "name": "exp-005-attention-qkvo-full",
    "purpose": "Controlled q/k/v/o versus q/v LoRA target comparison",
    "status": "complete",
    "parent_probe": "exp-005-attention-qkvo-probe",
    "comparison_parent": "exp-004c-full-data-batch1-lr5e-7",
    "changed_factor": {
        "name": "lora_attention_targets",
        "baseline": ["self_attn.q_proj", "self_attn.v_proj"],
        "experiment": targets,
    },
    "started_at_utc": started_at.isoformat(),
    "finished_at_utc": finished_at.isoformat(),
...
print("exp_005_full_training_ok: True")
