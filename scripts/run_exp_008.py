#!/usr/bin/env python3
"""Continue Experiment 006 with label-smoothed cross-entropy."""
...
PROBE_RESULT_PATH = PROJECT_ROOT / "artifacts" / "encoder" / "exp-008-probe" / "result.json"
TRAINER_OUTPUT_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-008" / "trainer-output"
RESULT_PATH = PROJECT_ROOT / "artifacts" / "encoder" / "exp-008" / "training_result.json"
...
def read_jsonl(path: Path, expected_hash: str) -> list[dict[str, object]]:
    if sha256(path) != expected_hash:
        raise SystemExit(f"Experiment 008 stopped: checksum mismatch for {path.name}.")
...
if Path(sys.executable).resolve() != (EXPECTED_ENV / "bin" / "python").resolve():
    raise SystemExit("Experiment 008 stopped: use .venv-encoder/bin/python.")
if not torch.backends.mps.is_available():
    raise SystemExit("Experiment 008 stopped: PyTorch MPS is unavailable.")
if RESULT_PATH.exists():
    raise SystemExit("Experiment 008 stopped: completed training result already exists.")
if not PROBE_RESULT_PATH.exists():
    raise SystemExit("Experiment 008 stopped: successful probe result is missing.")
...
if parent_result["weight_sha256"] != expected_parent_hash:
    raise SystemExit("Experiment 008 stopped: configured parent hash mismatch.")
if sha256(parent_weight_path) != expected_parent_hash:
    raise SystemExit("Experiment 008 stopped: parent model checksum mismatch.")
if (
    probe_result["label_smoothing"] != config["objective"]["label_smoothing"]
    or probe_result["class_weights"] != "uniform"
    or not math.isfinite(float(probe_result["training_loss"]))
):
    raise SystemExit("Experiment 008 stopped: probe result does not match configuration.")
...
if TRAINER_OUTPUT_DIR.exists():
    ...
    if list(TRAINER_OUTPUT_DIR.iterdir()) and resume_checkpoint is None:
        raise SystemExit(
            "Experiment 008 stopped: output exists without a resumable checkpoint."
        )
...
print("Open Model Training Lab — Experiment 008 BERT refinement")
...
print("loss: label_smoothed_cross_entropy")
...
if len(validation_history) != config["optimization"]["epochs"]:
    raise SystemExit("Experiment 008 stopped: expected two validation measurements.")
best_child = max(
    validation_history,
...
result = {
    ...
    "canonical_model_after_experiment": (
        "exp-008-pending-finalization" if promoted else "exp-006-selected-model"
    ),
...
print("exp_008_training_ok: True")
