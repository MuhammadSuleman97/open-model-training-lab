#!/usr/bin/env python3
"""Run a two-update, train-only Experiment 010 continuation probe."""
...
CONFIG_PATH = PROJECT_ROOT / "configs" / "exp-010-deberta-targeted-oversampling.json"
DATA_MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "encoder" / "exp-010" / "data" / "manifest.json"
...
RESULT_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-010-probe"
...
def stop(message: str) -> None:
    raise SystemExit(f"Experiment 010 probe stopped: {message}")
...
if not PARENT_MODEL_DIR.is_dir() or not PARENT_RESULT_PATH.is_file():
    stop("finalized Experiment 009d model is missing.")
...
print("Open Model Training Lab — Experiment 010 train-only continuation probe")
