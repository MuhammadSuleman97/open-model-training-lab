#!/usr/bin/env python3
"""Run a finite-loss MPS probe for the Exp013 hard-negative objective."""
...
RESULT_DIR = PROJECT_ROOT / "artifacts" / "encoder" / "exp-013-probe"
...
def stop(message: str) -> None:
    raise SystemExit(f"Experiment 013 probe stopped: {message}")
...
for optimizer_step in range(1, int(config["optimization"]["probe_steps"]) + 1):
    ...
    for micro_step in range(accumulation_steps):
        ...
        if len(batch_records) != batch_size:
            stop("probe batch indexing exceeded the deterministic probe rows.")
        encoded = tokenizer(
        ...
        labels_tensor = torch.tensor(
            [int(record["label"]) for record in batch_records],
            dtype=torch.long,
            device=device,
        )
        rival_tensor = torch.tensor(
            [
                label2id[record["hard_negative_label"]]
                if record["hard_negative_active"]
                else -1
    ...
    print(
        ...
        f"pairwise_loss: {loss_history[-1]['pairwise_loss']:.6f}, "
        f"active_rows: {active_count}"
    )

elapsed_seconds = time.monotonic() - started
RESULT_DIR.mkdir(parents=True, exist_ok=False)
result = {
    "experiment": "exp-013-probe",
    "parent_experiment": config["model"]["parent_experiment"],
    "parent_weight_sha256": parent_result["weight_sha256"],
    "probe_rows": probe_rows,
    "hard_negative_rows_in_probe": sum(record["hard_negative_active"] for record in probe_records),
...
print("Open Model Training Lab — Experiment 013 hard-negative MPS probe")
