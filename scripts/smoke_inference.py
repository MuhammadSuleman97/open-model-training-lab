#!/usr/bin/env python3
"""Run one deterministic prediction with the untouched baseline model."""
...
RESULT_PATH = PROJECT_ROOT / "experiments" / "smoke" / "model_smoke.json"
...
print("Open Model Training Lab — untouched-model smoke inference")
...
if last_response is None:
    raise SystemExit("Smoke inference failed: the model generated no response.")
...
result = {
    ...
    "prompt_tokens_per_second": round(last_response.prompt_tps, 3),
    "generation_tokens_per_second": round(last_response.generation_tps, 3),
    "peak_memory_gb": round(last_response.peak_memory, 3),
    "elapsed_seconds_including_load": round(elapsed_seconds, 3),
...
print(f"valid_label: {valid_label}")
print(f"prediction_correct: {prediction_correct}")
